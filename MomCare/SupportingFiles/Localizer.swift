...
class Localizer {
    ...
    class func backBuLocalized(sender : LocalizableButton ,arImg : String , enImg : String){
        if Language.currentLanguage() == "ar"{
            sender.setImage(UIImage(named: arImg), for: .normal)
        }else{
           sender.setImage(UIImage(named: enImg), for: .normal)
        }
    }
}
