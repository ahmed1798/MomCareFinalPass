...
extension WorkDatesVC {
    ...
    func creatToolBar(text : UITextField) {
        ...
        switch text {
        case startTime:
            todayButton = UIBarButtonItem(title: "Now".localized(), style: .plain, target: self, action: #selector(todayStartTimeBuPressed(sender:)))
        case startTime:
            todayButton = UIBarButtonItem(title: "Now".localized(), style: .plain, target: self, action: #selector(todayEndTimeBuPressed(sender:)))
        default:
            return
        }
        let doneButton = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(doneButtonPressed(sender:)))
            let label = UILabel(frame: CGRect(x: 0, y: 0, width: view.frame.width/3, height: 40))
        label.text = "Choose your Time".localized()
